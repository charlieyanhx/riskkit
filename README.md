...
```bash
pip install -e ".[dev]"
pytest -q          # 89 tests: pricing oracles, book schema, VaR/ES identities (closed form, worst
...
parameter files, and reconciled against the exchange's published margins. Every column
offset was pinned on CME's own bytes (`cme.20250912.c.pa2`, EOD cycle) against numbers known
independently; the two facts a reader of the file needs are that **array values are a
5-digit magnitude followed by the sign, loss-positive for a long**, and that the 81 line
carries scenarios 1–9 and the 82 line 10–16 plus the composite delta.
...
margin (ES-01 on 2025-09-12: $20,936 long / $20,051 short) and the legacy arrays in the
`.pa2` are labelled not authoritative. The engine also reproduces the file's own spread
charges: a Sep/Oct GC calendar scans to 0 and pays the $300 series charge; Sep vs Jun-26
pays the $1,000 tier charge; a short 3700 put's requirement is its scan risk plus the
premium it owes, a long put's is negative (excess) — all tested on the real slice.

Not implemented: inter-commodity spread credits, the table-driven delivery charge, and
SPAN 2 itself. The scan risk of a single-commodity book is exact; a multi-commodity book's
total is an upper bound (no inter-commodity credit).
...
data/span/         published_2025-09-12.csv — GC / ES / CL published margins with sources
...
parameter there). v0.3 (shipped): the SPAN engine above. v0.4: inter-commodity credits and the delivery
charge; live marks from the deskboard bus; `pricers` as the surface pricer.
